import { Upload, Network, Target, BarChart3, Table2, Award, BookOpen, ArrowLeft, ClipboardList, CheckSquare } from 'lucide-react'

const Sidebar = ({ currentStep, setCurrentStep, onBackToDashboard }) => {
  const menuItems = [
    { id: 'students', label: 'Students', icon: Upload, color: 'from-green-500 to-green-600' },
    { id: 'coMapping', label: 'CO-PO Mapping', icon: Network, color: 'from-blue-500 to-blue-600' },
    { id: 'assessmentSetup', label: 'Assessment Setup', icon: ClipboardList, color: 'from-pink-500 to-pink-600' },
    { id: 'marksEntry', label: 'Marks Entry', icon: CheckSquare, color: 'from-indigo-500 to-indigo-600' },
  ...
  const stepOrder = ['students', 'coMapping', 'assessmentSetup', 'marksEntry', 'kpi', 'reports', 'allDetails', 'results']

  return (
    <aside className="relative z-10 w-64 bg-gradient-to-br from-green-50/90 via-white to-blue-50/90 backdrop-blur-lg shadow-xl border-r-2 border-green-200 min-h-screen p-6 flex flex-col justify-between">
      <div>
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-4 py-2.5 mb-6 text-sm font-bold text-gray-700 hover:text-green-700 bg-white/60 hover:bg-green-50/80 rounded-xl transition-all border border-gray-200 shadow-sm"
        >
          <ArrowLeft size={16} />
          Back to Dashboard
        </button>
