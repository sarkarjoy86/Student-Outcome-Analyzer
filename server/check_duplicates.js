...
async function run() {
  await connectDB();
  const ids = rawData.map(d => d.studentId);
  const idsWithZero = rawData.map(d => "0" + d.studentId);

  const existingExact = await Student.find({ studentId: { $in: ids } });
  const existingZero = await Student.find({ studentId: { $in: idsWithZero } });

  console.log("Existing exact match count:", existingExact.length);
  console.log("Existing zero match count:", existingZero.length);
