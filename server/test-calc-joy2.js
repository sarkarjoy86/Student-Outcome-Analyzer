...
async function testCalcJoy2() {
  ...
  for (const en of enrollments) {
    ...
    if (marksDocs.length === 0) continue; // Skip offerings with no marks for this student
    ...
    for (let p = 1; p <= 12; p++) {
      ...
      if (mappedCOKeys.length > 0) {
        ...
        // ONLY sum credits if PO was mapped in this course
        poWeightedSums[targetPO] += maxCoAttainment * credits;
        poCreditSums[targetPO] += credits;
      }
    }

    console.log(`\nEvaluated Course: ${course.courseCode} (${credits} cr) - Grade: ${grade} (${gp})`);
    console.log(`  - CO Attainments:`, courseCOs);
    console.log(`  - PO Attainments:`, coursePOs);
  }

  const cgpa = totalAttemptedCredits > 0 ? (totalCreditPoints / totalAttemptedCredits) : 0;

  console.log(`\n========================================`);
  console.log(`CUMULATIVE GPA (CGPA): ${cgpa.toFixed(2)}`);
  ...
  for (let p = 1; p <= 12; p++) {
    ...
    console.log(`  - ${targetPO}: ${score.toFixed(2)}% (Mapped in ${w} credits)`);
  }
