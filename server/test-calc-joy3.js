...
async function testCalcJoy3() {
  ...
  for (const en of enrollments) {
    ...
    if (marksDocs.length === 0) continue;
    ...
    // 2. Merged CO-PO Mapping (master course + offering)
    const mergedMapping = {};
  ...
  console.log(`\n========================================`);
  console.log(`LONGITUDINAL PO ATTAINMENTS FOR JOY SARKAR (WITH MERGED MAPPING):`);
  for (let p = 1; p <= 12; p++) {
    const targetPO = `PO${p}`;
